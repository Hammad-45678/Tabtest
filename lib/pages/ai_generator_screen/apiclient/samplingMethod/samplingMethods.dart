...
class SamplingMethods {
  Future<List<SamplingMethodModel>> fetchItems(BuildContext context) async {
    List<SamplingMethodModel> items = [];
    ...
    try {
      http.Response response = await http
          .get(Uri.parse('https://cognise.art/api/samplers'), headers: headers);
      ...
      if (response.statusCode == 200) {
        ...
        items = itemsData
            .map((itemData) => SamplingMethodModel.fromJson(itemData))
            .toList();
        // Sort items based on the 'order' property

        // Print the fetched items
        for (var item in items) {
          print('Fetched Item: ${item.title}, ${item.descrption}, ');
        }
        return items;
      } else {
